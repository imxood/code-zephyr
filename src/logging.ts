...
type LogLevelKey = 'trace' | 'debug' | 'info' | 'note' | 'warning' | 'error' | 'fatal';
...
class OutputChannelManager implements vscode.Disposable {
     ...
     */
    dispose() { map(this._channels.values(), c => c.dispose()); }
}
...
function dataDir(): string { return path.join(userLocalDir(), 'zephyr'); }

function logFilePath(): string { return path.join(dataDir(), 'log.txt'); }
...
class SingletonLogger {
    ...
    private get _channel() { return channelManager.get('Zephyr'); }
        ...
        switch (level) {
            ...
            case LogLevel.Note:
                if (process.env['CMT_QUIET_CONSOLE'] !== '1') {
                    // tslint:disable-next-line
                    console.info('[Zephyr]', raw_message);
                }
                break;
            case LogLevel.Warning:
                // tslint:disable-next-line
                console.warn('[Zephyr]', raw_message);
                break;
            case LogLevel.Error:
            case LogLevel.Fatal:
                // tslint:disable-next-line
                console.error('[Zephyr]', raw_message);
                break;
        }
        // Write to the logfile asynchronously.
        this._logStream.then(strm => strm.write(raw_message + '\n')).catch(e => {
            // tslint:disable-next-line
            console.error('Unhandled error while writing Zephyr log file', e);
        });
...
export class Logger {
    ...
    showChannel() {
        const reveal_log = vscode.workspace.getConfiguration('zephyr').get<RevealLogKey>('revealLog', 'always');
...
function* map<In, Out>(iter: Iterable<In>, proj: (arg: In) => Out): Iterable<Out> {
